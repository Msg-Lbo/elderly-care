...
export default {
  ...
  methods: {
    ...
    initList(state = false, resetOrder = false, type = "load") {
      ...
      if (resetOrder) {
        this.list = [];
        this.$emit("load", { list: this.list, count: this.count });
      }
    ...
    getList(params, state, type) {
      ...
      api(params)
        .then((res) => {
		 ...
		 //  this.$api.myShoppingCart().then(res => {
		 //  	if (res.code == 200) {
		 ...
		 //  list.forEach(item => { 
			...
			//  	if (item.commodityList.id = this.defaultList[i].commodityId) {
			//  		item.commodityList.value = this.defaultList[i].number
			//  	}
			// 	console.log("this.defaultList[i]====>",this.defaultList[i])
			// 	console.log("item.commodityList====>",item.commodityList)
  ...
  computed: {
    ...
    listWrapStyle() {
      ...
      style.height = this.scrollViewHeight - uni.upx2px(offsetBottom) - this.btnHeight + "px";
