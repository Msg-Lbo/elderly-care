...
<script>
	...
	export default {
		...
		methods: {
			...
			getCropperImage() {
				let _this = this;
				let pathurl = this.vuex_baseUrl + '/common/upload'; //上传服务器地址
					...
					if (avatar) {
						...
						uni.uploadFile({
							...
							success: uploadFileRes => {
								...
								console.log(imgData, '转换图片为对象')
								if (imgData.code == 200) {
									uni.setStorageSync('avatar',imgData.fileName)
									//  获取到裁剪后的图片
									// wx.redirectTo({
									// 	url: './information?avatar=' + imgData.fileName
									// })
									uni.navigateBack()
								}
							},
